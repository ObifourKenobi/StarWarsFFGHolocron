...
---
Some Force users can telepathically guide allies around them, helping them to coordinate and achieve amazing feats.
...
**Easy difficulty Leader­ship check** as part of the pool. If [they] generate enough #StarWarsLightDark  to activate the power and succeed on the check, [they] can telepathically transmit a simple order to each character [they] affect with this power. This order is not mandatory, but the recipient comprehends it even if [they] do not understand the user’s languages.

**Control Upgrade:** Before attempting to activate this power, the user may suffer 4 strain to change the range of the Battle Meditation basic power to close on the planetary scale rather than engaged on the personal scale, for this use of the power only. If [they] do so, activating the range upgrades increases the range of the power by planetary scale range bands.

**Duration Upgrade:** The power gains the on­ going effect: Commit three Force dice after successfully activating the basic power. This power remains in effect on each affected target as long as the target stays within range of the power. If a target moves beyond the range of the power, the effects end for [them], but not for any other targets of the power. The user may not activate this multiple times.
...
**Range Upgrade:** Spend a Force point ( #StarWarsLightDark ) to increase the maximum range at which the Force user can affect targets through this power by a number of range bands equal to the number of Range upgrades purchased. The user may activate this multiple times, increasing the range by this number each time.

**Strength Upgrade:** Spend two Force points ( #StarWarsLightDark #StarWarsLightDark ) to add one additional success result to all affected characters' checks. The user may activate this multiple times, adding one additional success result to all checks each time.
# Source 
 [[Force and Destiny Core Rulebook|Force and Destiny Core Rulebook:284]]